...
def addNewPermission():
    ...
    per1 = Permission.objects.create(codename='view_all_post', name='Can View All Post',content_type=post_content)
    per2 = Permission.objects.create(codename='react_post', name='Can React Post', content_type=post_content)
    comment_content = ContentType.objects.get(app_label='comment', model='comment')
    per3 = Permission.objects.create(codename='react_comment', name='Can React Comment', content_type=comment_content)
    user_content = ContentType.objects.get(app_label='user', model='authen_user')
    per4 = Permission.objects.create(codename='view_profile', name='Can View Profile', content_type=user_content)
def addNewGroup():
